...
Asena.addCommand({ pattern: 'insta ?(.*)', fromMe: false, desc: IG_DESC }, async (message, match) => {
    ...
    if (type === 'image') { await message.sendMessage(data, MessageType.image, { caption: "💁🏻‍♀️Here is your Instagram image...⬇️Downloaded by 💘Meenakshi💘" }) }
    else if (type === 'video') { await message.sendMessage(data, MessageType.video, { caption: "💁🏻‍♀️Here is your Instagram video...⬇️Downloaded by 💘Meenakshi💘" }) }
});
//const axios = require('axios')
